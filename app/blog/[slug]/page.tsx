...
export default async function GuidePage({ params }: Props) {
  ...
  return <main className="bg-[#fafafa] text-slate-950">
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
      ...
      datePublished: guide.date ?? published, dateModified: guide.updated ?? guide.date ?? published, mainEntityOfPage: `https://resizefox.com/blog/${slug}`,
    ...
    <article className="mx-auto max-w-4xl px-5 py-12 sm:px-8 sm:py-16">
      ...
      <p className="mt-5 text-sm text-slate-500">By <Link href="/about" className="underline">ResizeFox</Link> · Published <time dateTime={guide.date ?? published}>{new Date(`${guide.date ?? published}T00:00:00Z`).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric", timeZone: "UTC" })}</time>{guide.updated && <> · Updated <time dateTime={guide.updated}>{guide.updated}</time></>}</p>
      ...
      <p className="mt-10 leading-8">See the tradeoffs yourself in the <Link href="/compression-lab" className="font-semibold text-orange-700 underline">Image Compression Lab</Link>: compare text, gradients and transparent graphics with downloadable results.</p>
