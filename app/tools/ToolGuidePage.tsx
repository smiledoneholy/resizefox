...
export default function ToolGuidePage({ guide, tool }: { guide: ToolGuide; tool: ReactNode }) {
  return <main className="min-h-screen bg-[#fafafa] text-slate-950">
    ...
    <section className="bg-slate-950 text-white"><div className="mx-auto max-w-5xl px-5 py-16 sm:px-8"><h2 className="text-3xl font-black">Related image tools</h2><div className="mt-7 grid gap-4 md:grid-cols-3">{guide.related.map(item => <Link key={item.href} href={item.href} className="rounded-2xl border border-slate-700 p-5 hover:border-orange-400"><h3 className="font-bold text-orange-400">{item.title}</h3><p className="mt-2 text-sm leading-6 text-slate-300">{item.description}</p></Link>)}</div><Link href="/tools" className="mt-7 inline-block font-bold text-orange-400 underline">Browse every image tool →</Link></div></section>
